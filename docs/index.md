...
4. When shipping, follow [deployment-guide.md](./deployment-guide.md) (update the ReaPack
   header, bump the version in its three homes, then regenerate `index.xml`).
...
6. A shipped change bumps the version in `@version`, `DM_RENAMER_VERSION` and `README.md`;
   `index.xml` is regenerated by `reapack-index`, never edited by hand.
